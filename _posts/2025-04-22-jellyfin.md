---
title: "Set Up a Mini PC as Your Own Jellyfin Media Server"
...
The mini PC comes with Windows 11, but since we're only using it to host Jellyfin, we don’t want the OS to take up too much storage. Ubuntu Server only takes up about **4GB** compared to **64GB** for Windows 11!
...
2. **Network Connection**: Confirm that Ethernet is connected. You should see an IP address assigned.

3. **Proxy Configuration**: Leave the proxy settings as default and select **Done**.

4. **Ubuntu Archive Mirror**: Leave this as default and select **Done**.
...
6. **Storage Review**: Review the configuration and select **Continue**.
...
8. **SSH Setup**: Check the box for **Install OpenSSH server** to enable remote access.

9. **Featured Server Snaps**: Select any other services you may want otherwise skip this.

10. **Finish Installation**: Let the install process complete. When prompted, select **"Reboot Now"** and remove the USB drive. If you don't remove the USB drive because 

11. **Login**: Log in using the username and password you just created.


12. Let's make sure the our packages are up-to-date
   ```bash
